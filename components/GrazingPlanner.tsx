...
type Paddock = {
  ...
  name: string;
  acres: number | null;
  forage_dm_lb_ac: number | null; // standing DM (lb/ac)
  util_pct: number | null;        // planned utilization %
...
export default function GrazingPlanner({ tenantId }: { tenantId: string }) {
  ...
  // herd (still local here; wire to your own API if you store herds)
  ...
  // paddocks come from Pasture Maintenance table
  const [paddocks, setPaddocks] = useState<Paddock[]>([]);

  /** Load paddocks from /api/paddocks (managed in Pasture Maintenance) */
  async function loadPaddocks() {
    try {
      setLoading(true);
      const rows: Paddock[] = await paddocksApi(tenantId, "listForPlanner");
      setPaddocks(rows || []);
    } catch (e: any) {
  ...
  const growthDMOverHorizon = useMemo(
    () => paddocks.reduce((a, p) => a + growthDmLbAcDay * Number(p.acres ?? 0) * days, 0),
  ...
  /** Save planner fields back to paddocks */
  async function savePlannerFields() {
    try {
      setLoading(true);
      const rows = paddocks.map((p) => ({
        id: p.id,
        acres: p.acres ?? null,
        forage_dm_lb_ac: p.forage_dm_lb_ac ?? null,
        util_pct: p.util_pct ?? null,
        rest_days: p.rest_days ?? null,
      }));
      await paddocksApi(tenantId, "bulkUpdatePlanner", { rows });
      setSaveMsg("Saved ✓");
      setTimeout(() => setSaveMsg(null), 1500);
    } catch (e: any) {
      alert(e.message || "Failed to save planner fields");
    } finally {
      setLoading(false);
    }
  }
  ...
  function updatePaddock(i: number, patch: Partial<Paddock>) {
    setPaddocks((prev) => prev.map((p, idx) => (idx === i ? { ...p, ...patch } as Paddock : p)));
  }
        ...
        <div className="grid md:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => setHerd({ ...herd, headcount: Number(e.target.value || 0) })}
            />
          </div>
          <div>
            ...
            <Input
              ...
              onChange={(e) => setHerd({ ...herd, avg_weight_lb: Number(e.target.value || 0) })}
            />
          </div>
          <div>
            ...
            <Input
              ...
              onChange={(e) => setHerd({ ...herd, intake_pct_bw: Number(e.target.value || 0) })}
            />
          </div>
        </div>

        {/* Paddocks table (managed in Pasture Maintenance) */}
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <Label className="font-medium">Paddocks (managed in Pasture Maintenance)</Label>
            <div className="flex items-center gap-2">
              <Link href="/pasture">
                <Button size="sm" variant="outline">Open Pasture Maintenance</Button>
              </Link>
              <Button
                size="sm"
                variant="outline"
                onClick={savePlannerFields}
                disabled={loading}
                title="Save acres/DM/utilization/rest back to paddocks"
              >
                {saveMsg || "Save"}
              </Button>
            </div>
                  ...
                  <tr key={p.id} className="border-t">
                    ...
                    <td className="p-2">
                      <Input
                        type="number"
                        value={p.acres ?? 0}
                        onChange={(e) => updatePaddock(i, { acres: Number(e.target.value || 0) })}
                      />
                    </td>
                    <td className="p-2">
                      <Input
                        ...
                        onChange={(e) => updatePaddock(i, { forage_dm_lb_ac: Number(e.target.value || 0) })}
                      />
                    </td>
                    <td className="p-2">
                      <Input
                        type="number"
                        value={p.util_pct ?? 0}
                        onChange={(e) => updatePaddock(i, { util_pct: Number(e.target.value || 0) })}
                      />
                    </td>
                    <td className="p-2">
                      <Input
                        type="number"
                        value={p.rest_days ?? 0}
                        onChange={(e) => updatePaddock(i, { rest_days: Number(e.target.value || 0) })}
                      />
                    </td>
                  </tr>
                ))}
                {paddocks.length === 0 && (
                  <tr><td className="p-2" colSpan={5}>No paddocks. Create them in Pasture Maintenance.</td></tr>
                )}
              </tbody>
            </table>
          </div>
