...
async function cattleApi<T = any>(action: string, body?: any): Promise<T> {
  ...
  try {
    ...
    // fall back to raw text for error
  }
  if (!res.ok || !json?.ok) {
...
export default function CattleByTag({ tenantId }: { tenantId: string }) {
  ...
  /* photos: upload without Supabase client (signed PUT flow, handled by /api/cattle) */
  async function uploadPhotos(files: FileList) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      try {
        // Ask the server for a signed, one-time upload URL to your bucket (server must implement this)
        const signed = await cattleApi<{
          path: string;
          upload_url: string; // pre-signed PUT URL
          public_url: string; // public (or signed) URL to view
        }>("requestUploadUrl", {
          tenant_id: tenantId,
          tag: safeTag,
          ext,
        ...
        // Register the photo in DB (service role)
        ...
        alert(
          e?.message ||
            "Upload failed. If you see 'Unknown action: requestUploadUrl', add that server action to /api/cattle."
        );
        break;
      }
    }
  ...
  async function deletePhoto(p: PhotoRow) {
    ...
    try {
      await cattleApi("deleteAnimalPhoto", {
        ...
        animal_id: editing.id,
        id: p.id,
        photo_path: p.photo_path, // optional server-side delete from storage
      });
      await loadPhotos(editing.id);
      await loadAnimals();
    } catch (e: any) {
  ...
  async function handleDrop(idx: number) {
    ...
    try {
      await cattleApi("reorderAnimalPhotos", {
        tenant_id: tenantId,
        animal_id: editing?.id,
        ordered_ids,
      });
  ...
  async function exportAnimalPdf() {
    ...
    if (treatsToPrint.length === 0) {
      ...
      treatsToPrint.forEach((t) => {
        doc.text(
          `${formatDate(t.treat_date)} — ${t.product || "Unspecified"}${
            t.dose ? " — " + t.dose : ""
          }${t.notes ? " — " + t.notes : ""}`,
          marginX,
          y
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setProcDraft({
                        ...procDraft,
                        live_weight_lb: Number(e.target.value || 0),
                      })
                    }
                  />
                </div>
