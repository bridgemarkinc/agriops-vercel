...
type Animal = {
  ...
  current_paddock_id?: number | null;   // FK
  current_paddock?: string | null;      // legacy (read-only if present)
  status?: string | null;
...
export default function CattleByTag({ tenantId }: { tenantId: string }) {
  ...
  const [uploadTotals, setUploadTotals] = useState<{ done: number; total: number }>({ done: 0, total: 0 });

  // new animal draft (uses paddock_id)
  ...
  async function loadPaddocks() {
    try {
      const rows = await paddocksApi<any[]>(tenantId, "listForPlanner");
      setPaddocks((rows || []).map((r) => ({ id: r.id, name: r.name })));
    } catch (e: any) {
      console.error("Failed to load paddocks:", e.message);
    }
  }
  ...
  async function loadProcessing(animalId: number) {
    try {
      const data = await cattleApi<Processing[]>("listProcessing", {
        tenant_id: tenantId,
        animal_id: animalId,
      });
      setProcessing(data || []);
    } catch (e: any) {
      alert(e.message || "Failed to load processing rows");
    }
  }

  async function loadPhotos(animalId: number) {
    try {
      const data = await cattleApi<PhotoRow[]>("listAnimalPhotos", {
        tenant_id: tenantId,
        animal_id: animalId,
      });
      setPhotos(data || []);
  ...
  useEffect(() => {
    if (!tenantId) return;
    Promise.all([loadPaddocks(), loadAnimals()]).catch(() => {});
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tenantId]);

  // reload when toggling archived
  useEffect(() => {
    if (!tenantId) return;
    loadAnimals().catch(() => {});
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  /* CRUD animals — uses current_paddock_id */
  async function saveAnimal() {
    ...
    const payload = {
      ...
      current_paddock_id: draft.current_paddock_id ?? null,
      status: draft.status || null,
    };
    try {
      await cattleApi("upsertAnimal", { payload });
      setDraft({
        tenant_id: tenantId,
        tag: "",
        name: "",
  ...
  async function updateAnimal(a: Animal) {
    ...
    const patch = {
      ...
      current_paddock_id: a.current_paddock_id ?? null, // ✅ FK
      status: a.status ?? null,
      death_date: a.death_date ?? null,
      death_notes: a.death_notes ?? null,
    };
    try {
      await cattleApi("updateAnimal", { id: a.id, tenant_id: tenantId, patch });
      await loadAnimals();
    } catch (e: any) {
  ...
  async function addWeight(animalId: number, weigh_date: string, weight_lb: number, notes?: string) {
    ...
    try {
      await cattleApi("addWeight", { tenant_id: tenantId, animal_id: animalId, weigh_date, weight_lb, notes });
  ...
  async function addTreatment(animalId: number, treat_date: string, product?: string, dose?: string, notes?: string) {
    ...
    try {
      await cattleApi("addTreatment", { tenant_id: tenantId, animal_id: animalId, treat_date, product, dose, notes });
      await loadDetail(animalId);
    } catch (e: any) {
  ...
  async function sendToProcessing() {
    ...
    try {
      await cattleApi("sendToProcessing", {
        tenant_id: tenantId,
      ...
      alert(e.message || "Failed to create processing row");
    }
  }

  async function updateProcessingRow(row: Processing, patch: Partial<Processing>) {
    try {
      await cattleApi("updateProcessing", { id: row.id, tenant_id: tenantId, patch });
      if (editing?.id) await loadProcessing(editing.id);
    } catch (e: any) {
      alert(e.message || "Failed to update processing row");
    }
  }

  /* photos: upload via signed PUT */
  async function uploadPhotos(files: FileList) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      try {
        const signed = await cattleApi<{ path: string; upload_url: string; public_url: string }>("requestUploadUrl", {
          tenant_id: tenantId,
          tag: safeTag,
          ext,
        ...
        const put = await fetch(signed.upload_url, { method: "PUT", headers: { "Content-Type": contentType }, body: file });
        if (!put.ok) {
          const t = await put.text().catch(() => "");
          throw new Error(`Upload PUT failed (${put.status}): ${t?.slice(0, 120)}`);
        }

        await cattleApi("addAnimalPhoto", {
          tenant_id: tenantId,
          animal_id: editing.id,
          tag: editing.tag,
          photo_url: signed.public_url,
        ...
        alert(e?.message || "Upload failed (ensure /api/cattle has requestUploadUrl).");
        break;
      }
    }
  ...
  async function setPrimaryPhoto(id: number, url: string) {
    if (!editing?.id) return;
    try {
      await cattleApi("setPrimaryPhoto", { tenant_id: tenantId, animal_id: editing.id, id });
      await loadPhotos(editing.id);
      await loadAnimals();
  ...
  async function deletePhoto(p: PhotoRow) {
    ...
    try {
      await cattleApi("deleteAnimalPhoto", {
        tenant_id: tenantId,
        animal_id: editing.id,
        id: p.id,
  ...
  async function handleDrop(idx: number) {
    ...
    try {
      await cattleApi("reorderAnimalPhotos", { tenant_id: tenantId, animal_id: editing?.id, ordered_ids });
    } catch (e: any) {
      alert(e.message || "Failed to save photo order");
    }
  }
    ...
    try {
      ...
      const mapped = rows
        .map((cols) => {
          ...
          let current_paddock_id: number | null = null;
          const idCol = get("current_paddock_id");
          if (idCol) {
            const n = Number(idCol);
            current_paddock_id = Number.isFinite(n) ? n : null;
          } else {
            const nameCol = get("current_paddock");
            if (nameCol) {
              const found = paddocks.find((p) => p.name.toLowerCase() === nameCol.toLowerCase());
              current_paddock_id = found ? found.id : null;
            }
          }
      ...
      const data = await cattleApi("bulkUpsertAnimals", { rows: mapped });
      setImportMsg(`Imported ${data?.imported ?? mapped.length} animals`);
      await loadAnimals();
    } catch (e: any) {
  ...
  function exportCSV() {
    const header = ["tenant_id", "tag", "name", "sex", "breed", "birth_date", "current_paddock_id", "status"];
  ...
  const treatmentSummary = useMemo(() => {
    const inRange = treats.filter((t) => (!from || t.treat_date >= from) && (!to || t.treat_date <= to));
  ...
  async function exportAnimalPdf() {
    ...
    if (favForPdf && favForPdf !== "primary") favorite = photos.find((p) => p.id === favForPdf);
    else favorite = photos.find((p) => p.is_primary) || photos[0];
    ...
    if (favorite?.photo_url) {
      ...
      if (dataUrl) {
        ...
        const paddockName = editing.current_paddock_id ? paddockMap.get(editing.current_paddock_id) || "" : "";
          ...
          <div className="border rounded-xl p-3 bg-white/70">
            <div className="grid grid-cols-2 gap-2">
              <div className="col-span-2">
                <Label>Tag *</Label>
                <Input value={draft.tag} onChange={(e) => setDraft({ ...draft, tag: e.target.value })} placeholder="e.g., BR123" />
              </div>
              <div>
                <Label>Name</Label>
              ...
              <div>
                ...
                <select
                  ...
                  value={draft.current_paddock_id ?? ""}
                  onChange={(e) =>
                    setDraft({ ...draft, current_paddock_id: e.target.value ? Number(e.target.value) : null })
                  }
                >
              ...
              <div className="col-span-2">
                <Button onClick={saveAnimal} className="w-full">Save Animal</Button>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 border rounded-xl p-3 bg-white/70">
            <div className="grid md:grid-cols-2 gap-2">
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  <Input placeholder="Filter by tag (e.g., BR)" value={search} onChange={(e) => setSearch(e.target.value)} />
                  ...
                  <Button
                    ...
                    title="Toggle to include animals with status 'deceased'"
                  >
            ...
            <div className="mt-3 border rounded-lg">
              <div className="flex items-center justify-between px-2 py-2 bg-slate-50 border-b">
                ...
                <div className="text-xs text-slate-500">Showing ~10 rows — scroll to see more</div>
              </div>
              ...
              <div className="max-h-[520px] overflow-y-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {animals.map((a) => {
                      const paddockName =
                        a.current_paddock_id != null
                          ? paddockMap.get(a.current_paddock_id) || ""
                          : (a.current_paddock || "");
                      return (
                        <tr key={a.id || a.tag} className="border-t">
                          <td className="p-2">
                            {a.primary_photo_url ? (
                              <img src={a.primary_photo_url} alt={a.tag} className="h-10 w-10 object-cover rounded-md border" />
                            ) : (
                          ...
                          <td className="p-2">{paddockName}</td>
                          <td className="p-2">{a.status}</td>
                          <td className="p-2 text-right">
                            <Button size="sm" variant="outline" onClick={() => startEdit(a)}>Open</Button>
                          </td>
                        </tr>
                      );
                    })}
                    {animals.length === 0 && (
                      <tr>
                        <td className="p-2" colSpan={9}>No cattle yet. Add your first animal on the left.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
        ...
        {editing && (
          <div className="border rounded-xl p-4 bg-white/80">
            <div className="flex items-center justify-between mb-2">
              <div className="text-lg font-semibold">Tag <span className="font-mono">{editing.tag}</span></div>
              <Button variant="outline" size="sm" onClick={() => setEditing(null)}>Close</Button>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-3">
              ...
              <div>
                ...
                <select
                  className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm"
                  value={editing.current_paddock_id ?? ""}
                  onChange={(e) => setEditing({ ...editing, current_paddock_id: e.target.value ? Number(e.target.value) : null })}
                >
                  <option value="">(none)</option>
                  {paddocks.map((p) => (
                    <option key={p.id} value={p.id}>{p.name}</option>
              ...
              <div>
                <Label>Status</Label>
                <Input value={editing.status || ""} onChange={(e) => setEditing({ ...editing, status: e.target.value })} />
              </div>

              {/* Deceased details */}
              <div className="md:col-span-3 mt-2 grid md:grid-cols-3 gap-3">
                <div>
                  <Label>Date of Death</Label>
                  <Input type="date" value={editing.death_date || ""} onChange={(e) => setEditing({ ...editing, death_date: e.target.value })} />
                </div>
                <div className="md:col-span-2">
                  <Label>Cause / Notes</Label>
                  <Input value={editing.death_notes || ""} onChange={(e) => setEditing({ ...editing, death_notes: e.target.value })} placeholder="optional" />
                </div>
              </div>
              ...
              <div className="md:col-span-3 flex gap-2 items-center">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    const next: Animal = { ...editing, status: "deceased", death_date: editing.death_date || date };
                    setEditing(next);
                    updateAnimal(next);
                  }}
                >
            ...
            <div className="mt-6 border rounded-xl p-4 bg-white/80">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) => setFavForPdf(e.target.value === "primary" ? "primary" : Number(e.target.value))}
                    title="Photo to use on PDF"
                  >
                    <option value="primary">Use Primary Photo</option>
                    {photos.map((p) => (
                      <option value={p.id} key={p.id}>#{p.id} {p.is_primary ? "(primary)" : ""}</option>
                    ))}
                  </select>
              ...
              <div
                onDragOver={(e) => { e.preventDefault(); if (!dzActive) setDzActive(true); }}
                onDragLeave={() => setDzActive(false)}
                onDrop={(e) => { e.preventDefault(); setDzActive(false); const files = e.dataTransfer?.files; if (files?.length) uploadPhotos(files); }}
                className={[
              ...
              >
                <div className="text-sm text-slate-700">Drag & drop images here, or click “Upload Photos”</div>
                <div className="text-xs text-slate-500 mt-1">JPG, PNG. You can also drag to reorder after uploading.</div>
              ...
              <div className="mt-3 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                {photos.map((p, idx) => (
                  ...
                  >
                    <img src={p.photo_url} alt={editing?.tag} className={`h-28 w-full object-cover rounded-lg border ${p.is_primary ? "ring-2 ring-emerald-500" : ""}`} />
                    <div className="absolute inset-x-1 bottom-1 flex gap-1 justify-center opacity-0 group-hover:opacity-100 transition">
                      {!p.is_primary && (
                        <button className="text-xs px-2 py-1 bg-white/90 border rounded" onClick={() => setPrimaryPhoto(p.id, p.photo_url)} title="Set as primary">
                          Set Primary
                        </button>
                      )}
                      <button className="text-xs px-2 py-1 bg-white/90 border rounded text-red-700" onClick={() => deletePhoto(p)} title="Delete photo">
                        Delete
                      </button>
                    </div>
                    {p.is_primary && (
                      <div className="absolute top-1 left-1 text-[10px] bg-emerald-600 text-white px-1.5 py-0.5 rounded">Primary</div>
                    )}
                    <div className="absolute top-1 right-1 text-[10px] bg-white/90 border px-1 rounded opacity-0 group-hover:opacity-100">↕︎ drag</div>
                  </div>
                ))}
                {photos.length === 0 && (
                  <div className="text-sm text-slate-600 col-span-full">No photos yet. Use drag-and-drop or “Upload Photos” to add images.</div>
                )}
              </div>
            </div>
            ...
            <div className="mt-6 border rounded-xl p-4 bg-white/80">
              ...
              <div className="grid md:grid-cols-5 gap-2">
                <div>
                  ...
                  <Input type="date" value={procDraft.sent_date || ""} onChange={(e) => setProcDraft({ ...procDraft, sent_date: e.target.value })} />
                </div>
                <div>
                  <Label>Processor</Label>
                  <Input value={procDraft.processor || ""} onChange={(e) => setProcDraft({ ...procDraft, processor: e.target.value })} />
                </div>
                <div>
                  <Label>Transport ID</Label>
                  <Input value={procDraft.transport_id || ""} onChange={(e) => setProcDraft({ ...procDraft, transport_id: e.target.value })} />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setProcDraft({ ...procDraft, live_weight_lb: Number(e.target.value || 0) })}
                  />
                </div>
                <div>
                  <Label>Notes</Label>
                  <Input value={procDraft.notes || ""} onChange={(e) => setProcDraft({ ...procDraft, notes: e.target.value })} />
                </div>
              ...
              <div className="overflow-auto mt-3 border rounded">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                      ...
                      <tr key={r.id} className="border-t">
                        ...
                        <td className="p-2 text-right">
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => updateProcessingRow(r, { status: r.status === "scheduled" ? "in_transit" : "scheduled" })}
                          >
                            Toggle Status
                          </Button>
                        </td>
                    ...
                    {processing.length === 0 && (
                      <tr>
                        <td className="p-2" colSpan={6}>No processing records yet.</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <div className="mt-6">
              ...
              <div className="grid md:grid-cols-2 gap-3">
                <div className="border rounded-lg p-3 bg-white/60">
                  ...
                  {adg !== null ? (
                    <div className="text-sm">ADG: <b>{adg.toFixed(2)}</b> lb/day</div>
                  ) : (
                    <div className="text-sm">Not enough weight records in range.</div>
                  )}
                  <div className="text-xs text-slate-600 mt-1">ADG uses earliest and latest weights in the selected date range.</div>
                </div>
    ...
    <div className="border rounded-lg p-3 bg-white/60">
      <div className="grid md:grid-cols-4 gap-2">
        ...
        <div className="md:col-span-4">
          <Button onClick={() => onAdd(animalId, date, Number(w || 0), notes || undefined)}>Add Weight</Button>
        </div>
      </div>
