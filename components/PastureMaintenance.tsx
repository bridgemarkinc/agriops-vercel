...
type Paddock = {
  ...
  head_count?: number | null; // optional if your API returns it
};
...
async function addPaddock(tenantId: string, paddock: {
  name: string;
  acres?: number;
  ...
  const res = await fetch("/api/paddocks", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "upsertPaddock", tenant_id: tenantId, payload: paddock }),
  });
  const json = await res.json();
  if (!res.ok || !json.ok) throw new Error(json.error || "failed to add paddock");
  return json.data;
}
...
export default function PastureMaintenance({ tenantId }: { tenantId: string }) {
  ...
  async function loadPaddocks() {
    try {
      setLoadingPads(true);
      // Use "list" unless your API implements "listWithCounts"
      const rows: Paddock[] = await paddocksApi("list", { tenant_id: tenantId });
      setPaddocks(rows || []);
    } catch (e: any) {
        ...
        <div className="border rounded-xl overflow-hidden">
          <div className="flex items-center justify-between px-3 py-2 bg-slate-50 border-b">
            <div className="text-sm font-medium">
              ...
              {typeof herdHead === "number" ? <> • Herd: {herdHead} head</> : null}
            </div>
            <div className="flex items-center gap-2">
              <Button
                ...
                variant="outline"
                onClick={async () => {
                  const name = prompt("Paddock name?");
                  if (!name) return;
                  try {
                    await addPaddock(tenantId, { name, acres: 5 });
                    await loadPaddocks();
                  } catch (e: any) {
                    alert(e.message || "Failed to add paddock");
                  }
                }}
              >
                + Add Paddock
                ...
                <tr key={p.id} className="border-t">
                  ...
                  <td className="p-2">
                    <Input
                      ...
                      onChange={(e) =>
                        setPaddocks((prev) =>
                          prev.map((x) => (x.id === p.id ? { ...x, zone: e.target.value } : x))
                        )
                      }
                      placeholder="zone"
                    />
        ...
        {activePad && (
          <div className="border rounded-xl p-4 bg-white/80">
            <div className="flex items-center justify-between">
              <div className="text-lg font-semibold">{activePad.name} — Seeding & Amendments</div>
