...
// LIST with default exclusion of 'deceased'
case "listAnimals": {
  const tenant_id = String(body?.tenant_id || "");
  const search = (body?.search || "").trim();
  const include_deceased = !!body?.include_deceased;   // pass true to see archived
  if (!tenant_id) {
  ...
  if (!include_deceased) {
    q = q.neq("status", "deceased");
  }
  if (search) {
    q = q.ilike("tag", `%${search}%`);
  }
...
// UPSERT (create or update by natural key or id)
// Accepts death_date and death_notes
case "upsertAnimal": {
  ...
  const row = {
    ...
    current_paddock: p.current_paddock ?? null,   // or paddock_id if you’ve migrated
    status: p.status ?? "active",
    death_date: p.death_date ?? null,
    death_notes: p.death_notes ?? null,
  };

  const { data, error } = await supa
    .from("agriops_cattle")
    .upsert(row as any, { onConflict: "tenant_id,tag" }) // adjust onConflict to your constraint
    .select()
    .maybeSingle();
...
// UPDATE (by id) – allows changing status to 'deceased' and storing death_* fields
case "updateAnimal": {
  ...
  const upd = {
    ...
    current_paddock: patch.current_paddock ?? null, // or paddock_id if you’ve migrated
    status: patch.status ?? null,
    death_date: patch.death_date ?? null,
    death_notes: patch.death_notes ?? null,
  };
